...
export class ZohoService {
  ...
  async createEvent(eventData: {
    ...
    try {
      ...
      // O Objeto exato exigido pela Zoho
      const eventObj: any = {
        title: eventData.title,
        description: finalDescription,
        dateandtime: {
          timezone: 'America/Sao_Paulo', // Adicionado fuso horário explícito para segurança extra
      ...
      // O PULO DO GATO: Codificar o JSON como URL (Formulário)
  ...
  async updateEvent(
    ...
    try {
      ...
      // 1. Busca rápida para pegar o ETAG atualizado
      ...
      // 2. Envia o PUT usando a string codificada e o cabeçalho 'etag'
  ...
  async deleteEvent(eventId: string) {
    ...
    try {
      ...
      // 2. Envia o DELETE com o ETAG no cabeçalho
      await axios.delete(eventUrl, {
        headers: {
          Authorization: `Bearer ${token}`,
          etag: String(etag),
          'If-Match': String(etag), // Usado para garantir compatibilidade
        },
      });
